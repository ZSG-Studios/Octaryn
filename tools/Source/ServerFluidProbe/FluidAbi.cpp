...
struct Context {const FluidRules& rules;bool stone_world{};};
uint16_t generated(void* opaque,const octaryn_server_block_position*) {
  const auto& c=*static_cast<Context*>(opaque);return c.stone_world?c.rules.stone:AirBlock;
}
...
void check_fluid_abi(const FluidRules& rules) {
  configurations(rules);invalid_time(rules);cascades(rules,false);cascades(rules,true);
