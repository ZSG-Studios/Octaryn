...
struct World {
  ...
  uint16_t get(BlockPosition p) const {
    auto it=blocks.find({p.x,p.y,p.z});return it==blocks.end()?AirBlock:it->second;
  }
...
void check_scheduler(const FluidRules& rules) {
  timing(rules);retries_and_budgets(rules);regions_and_repair(rules);native_apply(rules);
