...
internal static class NeighborhoodValidation
{
    public static void ValidateSnapshot()
    {
        ...
        ProbeAssertions.Require(snapshot.NeighborhoodBlock(1, ChunkNeighborhoodSnapshot.Height - 1, 1, 0, 1, 0) == BlockId.Air, "snapshot missing upper section is air");
        ...
        var worldBottomStore = new BlockPresentationStore();
        ...
        var worldBottom = worldBottomStore.CaptureNeighborhood(
            new PresentationChunkKey(0, ChunkConstants.WorldMinY / ChunkNeighborhoodSnapshot.Height, 0),
            NeighborhoodBoundaryBlocks.StreamWindowEdge);
        ...
        var worldTopStore = new BlockPresentationStore();
        ...
        var worldTop = worldTopStore.CaptureNeighborhood(
            new PresentationChunkKey(0, (ChunkConstants.WorldMaxYExclusive - 1) / ChunkNeighborhoodSnapshot.Height, 0),
            NeighborhoodBoundaryBlocks.StreamWindowEdge);
        ProbeAssertions.Require(worldTop.LocalBlock(1, 1, 1, ChunkNeighborhoodSnapshot.Height - 1, 1).Value == 22, "snapshot captures centered world top block");
        ProbeAssertions.Require(worldTop.NeighborhoodBlock(1, ChunkNeighborhoodSnapshot.Height - 1, 1, 0, 1, 0) == BlockId.Air, "snapshot above world is air");
    }
    ...
    public static void ValidateFaceVisibility()
    {
        ...
        ProbeAssertions.Require(store.Apply(new BlockPosition(4, ChunkNeighborhoodSnapshot.Height - 1, 4), new BlockId(1)), "top face center block");
        ...
        ProbeAssertions.Require(ContainsFace(rules, snapshot, 4, ChunkNeighborhoodSnapshot.Height - 1, 4, Direction.PositiveY), "missing upper section exposes top face");
        ProbeAssertions.Require(!ContainsFace(rules, snapshot, 5, 0, 5, Direction.NegativeY), "lower section block hides bottom face");

        var airBelowStore = new BlockPresentationStore();
