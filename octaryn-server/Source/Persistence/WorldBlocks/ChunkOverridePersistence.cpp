...
constexpr uint32_t CurrentVersion = 2u;
...
bool upgrade_chunk_override_file(chunk_override_file &file) {
  ...
  for (const auto &block : file.blocks) {
    ...
    saw_local_only |= looks_like_local_coordinates && !already_world_coordinates;
    saw_world_only |= already_world_coordinates && !looks_like_local_coordinates;
  }
...
chunk_override_file file_from_abi(
