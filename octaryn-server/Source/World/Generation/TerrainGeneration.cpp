...
int32_t octaryn_server_terrain_plan_column(
  ...
  if (rules == nullptr || plan == nullptr || rules->generator_revision != 3) {
    return -1;
  }
...
int32_t octaryn_server_terrain_generated_block(
  ...
  if (rules->generator_revision != 3) return -1;
...
int32_t octaryn_server_terrain_clear_matching_overrides(
  ...
  if (block_store == nullptr || rules == nullptr || rules->generator_revision != 3) {
    return 0;
  }
