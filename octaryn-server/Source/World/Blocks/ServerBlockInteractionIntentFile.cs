...
internal sealed class ServerBlockInteractionIntentFile
{
    ...
    public IReadOnlyList<ServerBlockInteractionCommandFile> Commands { get; init; } = [];

    public bool IsSupported => Version == 1 &&
        FrameIndex > 0 &&
        Commands.Count <= ServerClientBlockCommandQueue.MaxPendingCommands &&
        Commands.All(command => command.IsSupported);
}

internal sealed class ServerBlockInteractionCommandFile
{
