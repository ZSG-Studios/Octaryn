...
constexpr std::uint32_t ChunkCount = FaceMaskProbeChunkCount;
  ...
  for (std::uint32_t z = 0; z < W; ++z) {
    for (std::uint32_t y = 0; y < W; ++y) {
      for (std::uint32_t x = 0; x < W; ++x) {
        const std::uint32_t index = voxel_index(x, y, z);
        const std::uint32_t palette_index =
            chunk_kind == CheckerboardChunk ? ((x + y + z) & 1u) : (x & 3u);
        const std::uint32_t bit_offset = index * 2u;
        bytes[bit_offset >> 3u] |=
...
bool append_chunk(FaceMaskBatch &batch, GpuChunkPayload chunk) {
...
FaceMaskBatch build_face_mask_batch() {
  FaceMaskBatch batch;
...
bool bind_resources(gfx::IComputeCommandEncoder *encoder,
                    ...
                    const std::array<gfx::IResourceView *, 5> &views,
...
SlangRhiVoxelFaceMaskProbeResult make_result(const char *status) {
  return {true, false, false, false, false, false, false, false, false, false,
          false, false, false, 0u,    0u,    0u,    0u,    status};
}
...
SlangRhiVoxelFaceMaskProbeResult probe_slang_rhi_voxel_face_masks() {
  ...
  const FaceMaskBatch batch = build_face_mask_batch();
  const auto expected = build_slang_rhi_voxel_face_mask_expected();
  if (batch.headers.size() != ChunkCount || batch.palette_entries.size() != 9u ||
      batch.payload_bytes.empty()) {
    probe.status = "face_mask_batch_invalid";
    return probe;
  }
  ...
  const std::size_t mask_bytes = expected.masks.size() * sizeof(std::uint32_t);
  constexpr std::size_t CountBytes = ChunkCount * sizeof(std::uint32_t);
  ...
  if (!create_slang_compute_pipeline(device, FaceMaskShaderPath, pipeline)) {
  ...
  Slang::ComPtr<gfx::IBufferResource> count_buffer;
  ...
  std::array<Slang::ComPtr<gfx::IResourceView>, 5> owned_views;
  if (!create_buffer_view(device, header_buffer,
      ...
      !create_buffer_view(device, mask_buffer,
                          ...
                          owned_views[3]) ||
      !create_buffer_view(device, count_buffer,
  ...
  std::array<gfx::IResourceView *, 5> views = {
      owned_views[0], owned_views[1], owned_views[2], owned_views[3],
      owned_views[4]};
  Slang::ComPtr<gfx::IShaderObject> root;
  Slang::ComPtr<gfx::IShaderObject> entry_point;
  if (!bind_resources(encoder, pipeline, views, root, entry_point)) {
    encoder->endEncoding();
    encoder->release();
  ...
  if (!result_succeeded(encoder->dispatchCompute(ChunkCount, 1, 1))) {
    encoder->endEncoding();
    encoder->release();
  ...
  encoder->bufferBarrier(count_buffer, gfx::ResourceState::UnorderedAccess,
                         gfx::ResourceState::CopySource);
  encoder->endEncoding();
  encoder->release();
  ...
  std::vector<std::uint32_t> actual_masks(expected.masks.size());
  std::vector<std::uint32_t> actual_counts(ChunkCount);
  if (!readback_u32(device, mask_buffer, actual_masks) ||
      !readback_u32(device, count_buffer, actual_counts)) {
    probe.status = "face_mask_readback_failed";
    return probe;
  }
  probe.empty_faces = actual_counts[EmptyChunk];
  probe.uniform_faces = actual_counts[UniformChunk];
  probe.checkerboard_faces = actual_counts[CheckerboardChunk];
  probe.mixed_faces = actual_counts[MixedChunk];
  probe.readback_valid = actual_masks == expected.masks;
  for (std::uint32_t i = 0; i < ChunkCount; ++i) {
    probe.readback_valid &= actual_counts[i] == expected.counts[i];
  }
  if (!probe.readback_valid) {
    probe.status = "face_mask_readback_mismatch";
    return probe;
  }
  ...
  probe.status = "gpu_voxel_face_masks_validated";
  return probe;
#else
  return {false, false, false, false, false, false, false, false, false, false,
          false, false, false, 0u,    0u,    0u,    0u,    "runtime_unavailable"};
#endif
}
