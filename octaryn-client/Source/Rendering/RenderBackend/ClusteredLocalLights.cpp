...
bool world_local_clusters(WorldRenderer& r,rhi::ICommandEncoder* commands) {
  ...
  root=pass->bindPipeline(s.tile_sort_pipeline);ok=root!=nullptr;
  if(ok) {
    rhi::ShaderCursor c(root);
    ok=world_rhi_ok(c["localTileCounts"].setBinding(rhi::Binding(s.tile_counts)))&&
      world_rhi_ok(c["localTileLights"].setBinding(rhi::Binding(s.tile_lights)))&&
      world_rhi_ok(c["localTileSettings"].setData(tiles,sizeof(tiles)));
  }
  if(ok)pass->dispatchCompute(tiles[0],tiles[1],1);
  pass->end();
