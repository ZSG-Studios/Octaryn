...
void ddgi_invalidate(DDGISystem& s,const std::array<float,3>& minimum,const std::array<float,3>& maximum) {
  ...
  for(unsigned i=0;i<s.control_data.size();++i) {
    ...
    for(unsigned axis=0;axis<3;++axis) {
      const float position=float(s.control_data[i].cell[axis])*s.config.spacing;
...
void ddgi_schedule(DDGISystem& s,const std::array<float,3>& camera) {
  ...
  for(unsigned axis=0;axis<3;++axis)
    s.origin[axis]=int(std::floor(camera[axis]/s.config.spacing))-int(counts[axis]/2);
  ...
  for(unsigned i=0;i<order.size();++i) {
    float distance2=0;
    for(unsigned axis=0;axis<3;++axis) {
      const float delta=float(s.control_data[i].cell[axis])*s.config.spacing-camera[axis];distance2+=delta*delta;
    }
  ...
  std::partial_sort(order.begin(),order.begin()+budget,order.end(),[&](unsigned a,unsigned b){
