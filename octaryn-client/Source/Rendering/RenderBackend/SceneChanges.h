...
public:
  ...
  void notify_column(std::int32_t x,std::int32_t z,int min_y,int height,SceneChangeKind kind) {
    const auto revision=++revision_;
    changes_[(revision-1)%changes_.size()]={revision,x,z,min_y,height,kind};
  }
