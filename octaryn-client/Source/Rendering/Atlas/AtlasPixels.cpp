// Ported from upstream Octaryn 3557cbf source/render/atlas/upload/filter.cpp
...
void atlas_dilate_transparent_rgb(Uint8* pixels, int size)
{
    Uint8 source[ATLAS_BLOCK_WIDTH * ATLAS_BLOCK_WIDTH * 4] = {0};
        ...
        for (int x = 0; x < size; ++x)
        {
            ...
            for (int oy = -1; oy <= 1; ++oy)
            {
                ...
                for (int ox = -1; ox <= 1; ++ox)
                {
                    ...
                    if (sample[3] < ATLAS_ALPHA_CUTOFF_U8)
                    {
                        continue;
                    }
...
void atlas_downsample_tile_rgba(const Uint8* src, int src_size, Uint8* dst)
{
