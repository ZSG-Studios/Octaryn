// Ported from upstream Octaryn 3557cbf source/render/atlas/upload/mips.cpp
...
void downsample_data_rgba(const Uint8* src, int src_size, Uint8* dst, atlas_mip_kind_t kind)
{
    if (kind == ATLAS_MIP_LABPBR_NORMAL)
    {
        downsample_labpbr_normal(src, src_size, dst);
        return;
    }
    if (kind == ATLAS_MIP_LABPBR_SPECULAR)
    {
...
void atlas_pack_layer_mips(
    Uint8* data,
    Uint32* offset,
    Uint8* current,
    Uint8* next,
    Uint32 mip_levels,
    atlas_mip_kind_t kind)
{
    float target_coverage = 0.0f;
    if (kind == ATLAS_MIP_ALBEDO)
    {
        atlas_dilate_transparent_rgb(current, ATLAS_BLOCK_WIDTH);
        target_coverage =
            atlas_alpha_coverage(current, ATLAS_BLOCK_WIDTH * ATLAS_BLOCK_WIDTH);
    }

    Uint8* level_pixels = current;
    Uint8* next_pixels = next;
    int size = ATLAS_BLOCK_WIDTH;
    for (Uint32 level = 0; level < mip_levels; ++level)
    {
        ...
        *offset += byte_count;
        if (level + 1 < mip_levels)
        {
            ...
            if (kind == ATLAS_MIP_ALBEDO)
            {
                atlas_downsample_tile_rgba(level_pixels, size, next_pixels);
                atlas_dilate_transparent_rgb(next_pixels, next_size);
                atlas_preserve_alpha_coverage(next_pixels, next_size * next_size, target_coverage);
            }
            else
            {
