...
column_mesh_plan_entry make_entry(int32_t chunk_x, int32_t chunk_z,
                                 const chunk_view &view,
  ...
  return {
      ...
      action,
      action == column_mesh_plan_action::build &&
          chunk_is_urgent(chunk_x, chunk_z, options),
  };
}

int entry_sort_key(const column_mesh_plan_entry &entry) {
  if (entry.action == column_mesh_plan_action::build && entry.urgent) {
    return 0;
  }
  if (entry.action == column_mesh_plan_action::build) {
    return 1;
  }
  if (entry.action == column_mesh_plan_action::preserve) {
    return 2;
  }
  return 3;
}

void add_current_entries(column_mesh_plan &plan,
                         const column_mesh_plan_options &options) {
  ...
  for (int32_t chunk_z = plan.current_view.origin_z;
    ...
    for (int32_t chunk_x = plan.current_view.origin_x;
      ...
      const auto action = preserved ? column_mesh_plan_action::preserve
                                    : column_mesh_plan_action::build;
      column_mesh_plan_entry entry =
...
void add_clear_entries(column_mesh_plan &plan,
                       const column_mesh_plan_options &options) {
...
void sort_plan_entries(std::vector<column_mesh_plan_entry> &entries) {
  std::sort(entries.begin(), entries.end(),
            [](const column_mesh_plan_entry &left,
               const column_mesh_plan_entry &right) {
              const int left_action = entry_sort_key(left);
              const int right_action = entry_sort_key(right);
              if (left_action != right_action) {
                return left_action < right_action;
              }
              if (left.action == column_mesh_plan_action::clear &&
                  right.action == column_mesh_plan_action::clear &&
