...
rendering::PlayerPose player_presentation(const LocalPlayerPose& pose,const WorldControls& controls,
    const rendering::WorldCamera& camera,double attack_until,uint64_t attack_sequence) {
  ...
  result.yaw=controls.yaw;result.source_seconds=pose.source_seconds;
  ...
  if(pose.source_seconds<attack_until) {result.clip=Clip::Attack;result.action_sequence=attack_sequence;}
