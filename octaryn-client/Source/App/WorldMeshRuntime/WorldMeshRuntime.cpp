...
bool run_prebuilt_world_mesh_upload(
  ...
  scheduled_world_mesh_update scheduled{gpu_device, &visible_frame, &mesh_buffers,
                                        update, frame_index, source};
  const octaryn_native_schedule_resource_access upload_accesses[] = {
      {"world_mesh.update_frame", OCTARYN_NATIVE_SCHEDULE_ACCESS_READ},
      {"gpu.world_mesh_upload", OCTARYN_NATIVE_SCHEDULE_ACCESS_WRITE}};
  ...
  const int runtime_result =
      octaryn_native_schedule_runtime_execute(runtime.handle, &job, 1u, &report);
  if (g_log != nullptr) {
    std::fprintf(
        g_log,
        "live_native_schedule_runtime frame=%" PRIu64
        ...
        " build_ms=%.3f upload_ms=%.3f chunks=%zu opaque_faces=%zu"
        " sprite_vertices=%zu result=%d\n",
        frame_index, runtime_result == 0 ? 1 : 0, source,
        report.submitted_jobs, report.completed_jobs, report.worker_jobs,
        report.main_thread_jobs, report.execution_waves,
        report.failed_job_index, frame_profile_elapsed_ms_since(start_ticks),
        scheduled.build_ms, scheduled.upload_ms, update.chunks.size(),
        update.opaque_faces.size(), update.sprite_vertices.size(),
        runtime_result);
    std::fflush(g_log);
  }
  if (runtime_result != 0) {
