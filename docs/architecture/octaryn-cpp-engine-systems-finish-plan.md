...
- Moved stale chunk-column override file pruning and `chunk_<cx>_<cz>.json` filename parsing into native server world-persistence code; managed chunk-column store now passes the native column plan to owner code before writing current sidecars.
