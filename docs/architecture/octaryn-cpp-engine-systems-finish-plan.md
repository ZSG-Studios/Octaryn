...
- Moved server persistence root/path selection into native server world-persistence path policy; managed player/world-block persistence now passes raw environment values through interop and no longer owns fallback path composition or aggregate sidecar directory derivation.
