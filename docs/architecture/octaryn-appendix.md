...
`docs/architecture/octaryn-master-plan.md` is the canonical Octaryn platform master plan. Root `plan.md` is the canonical Slang RHI GPU voxel renderer cutover plan. This file is an appendix and migration checklist. If this file conflicts with the master plan or renderer `plan.md`, update this file to match the active source.
...
- The core host baseline is a flying camera with no default player physics and flat blank terrain. Target voxel-renderer worlds are 1024 blocks tall through 32 vertically stacked 32-block chunks per root `plan.md`; existing 256-height, retired half-height, or chunk-edge-derived height constants are migration debt, not the destination model.
...
| 2 | Slang RHI material-table rendering | `references/old-architecture/source/render/atlas/`, `references/old-architecture/source/render/world/`, atlas shaders | client rendering, Slang shaders, basegame asset tooling | Old SDL texture/atlas rendering is quarantined. Rebuild material upload as Slang RHI-owned texture arrays or bindless-style material tables with mips, animation metadata, and PBR validation. |
