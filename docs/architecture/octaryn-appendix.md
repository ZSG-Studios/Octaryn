...
| 5 | World save metadata, player save, and chunk override format | `old-architecture/source/core/persistence/` | server persistence with shared compatibility contracts | World block overrides, time, and server-owned player save file round trips are present; world metadata, chunk file/cache semantics, and migration replay remain open. |
