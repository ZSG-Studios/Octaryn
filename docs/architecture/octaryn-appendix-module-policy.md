...
- `octaryn-basegame/Data/Module/octaryn.basegame.module.json` is the checked-in bundled module package descriptor. It mirrors `ModuleRegistration.Manifest`, is copied to client/server bundles, and is compared by `tools/validation/Octaryn.ModuleManifestProbe/` so package metadata can become the future discovery source without replacing the current in-process registration path yet.
