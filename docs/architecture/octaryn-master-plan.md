...
| Real native atlas upload/render path | `old-architecture/source/render/atlas/`, `old-architecture/source/render/world/`, shader atlas use | `octaryn-client/Source/Rendering/` and `octaryn-client/Shaders/` | Client launch validation now samples bundled color-atlas tiles through SDL texture rendering; full SDL_GPU 2D array upload, normal/specular material sampling, shader binding, and mip/PBR validation remain open. |
