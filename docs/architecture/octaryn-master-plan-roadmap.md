...
| Slang RHI material-table render path | `references/old-architecture/source/render/atlas/`, `references/old-architecture/source/render/world/`, shader atlas use | `octaryn-client/Source/Rendering/`, `octaryn-client/Shaders/`, and basegame asset tooling | Old SDL texture rendering and SDL GPU atlas upload are quarantined. Rebuild material upload as Slang RHI-owned texture arrays or bindless-style material tables with mips, animation metadata, and PBR validation. |
