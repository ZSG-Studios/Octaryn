...
- Bundled singleplayer server readiness runs with `tools/build/cmake_build.sh debug-linux --target octaryn_client_server_app_launch_probe`, requires the server app to emit `octaryn_server_ready=1` after activation and one host tick, and logs under `logs/server/octaryn_client_server_app_launch_probe-debug-linux.log`.
