...
- The graphical client launcher is a client-owned native executable copied into `build/<preset>/client/bundle/` as `Octaryn.Client`. `octaryn_validate_client_app_launch_probe` runs it with SDL's dummy video driver, a bounded frame count, and a validation-only presentation snapshot, then validates the log for window startup, managed client host initialization, native-drained presentation drawing, block-frame presentation, ticks, and clean shutdown without starting server authority from client code.
