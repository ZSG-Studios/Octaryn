...
- The graphical client launcher is a client-owned native executable copied into `build/<preset>/client/bundle/` as `Octaryn.Client`. `octaryn_run_client_app_launch_probe` runs it with SDL's dummy video driver and a bounded frame count to validate window startup, managed client host initialization, ticks, and clean shutdown without starting server authority from client code.
