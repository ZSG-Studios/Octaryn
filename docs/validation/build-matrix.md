...
- Server block-store native validation includes `octaryn_server_block_store_probe`, a C++ probe that validates centered world bounds, floor chunk/local mapping, override edits, preserved air overrides, deterministic snapshots, chunk-column snapshots, load semantics, generated-override cleanup, native chunk stream count/fill semantics, block-change queue drain semantics, client block-command queue semantics, client block-interaction hit/reach/adjacency semantics, and replication-change packing. The managed server block store/change queue/client command queue/chunk stream provider are narrow native-handle glue over the loadable `octaryn_server_block_store` owner library, which is copied into the server bundle and injected into managed validation probes with `OCTARYN_SERVER_BLOCK_STORE_LIBRARY`.
- Server player-simulation native validation includes `octaryn_server_player_simulation_probe`, a C++ probe that validates native spawn surface alignment, old-architecture walk/fly movement constants, jump/gravity behavior, AABB wall collision, and idle/no-input velocity clearing. The managed server player controller now uses this native owner library for movement/collision and idle updates while keeping persistence and live-debug logging in server glue.
...
- Server world-persistence native validation includes `octaryn_server_world_persistence_probe`, a C++ probe that validates native edited-block chunk-column origin planning and per-column block ordering for the world-block persistence backend. Managed persistence remains file/JSON glue over the loadable `octaryn_server_world_persistence` owner library, which is copied into the server bundle and injected into managed validation probes with `OCTARYN_SERVER_WORLD_PERSISTENCE_LIBRARY`.
