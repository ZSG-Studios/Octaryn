...
- The live bundled-server stream bridge skips duplicate metadata-only writes for windows it has already emitted; sustained 32-distance probes should show unchanged-window skips in the server log instead of recurring large chunk-stream JSON rewrites.
