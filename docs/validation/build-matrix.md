...
- Server world-persistence native validation includes `octaryn_server_world_persistence_probe`, a C++ probe that validates native edited-block chunk-column origin planning, per-column block ordering, chunk override directory scanning, stale sidecar pruning, sidecar directory writing, world-block aggregate/sidecar load-source and save coordination, gzip round trips, player save JSON read/write, world-time JSON read/write, and world metadata JSON read/write behavior. The same loadable `octaryn_server_world_persistence` owner library is copied into the server bundle and injected into managed validation probes with `OCTARYN_SERVER_WORLD_PERSISTENCE_LIBRARY`.
