...
- Server world-persistence native validation includes `octaryn_server_world_persistence_probe`, a C++ probe that validates native edited-block chunk-column origin planning, per-column block ordering, gzip round trips, player save JSON read/write, and world-time JSON read/write behavior. The same loadable `octaryn_server_world_persistence` owner library is copied into the server bundle and injected into managed validation probes with `OCTARYN_SERVER_WORLD_PERSISTENCE_LIBRARY`.
