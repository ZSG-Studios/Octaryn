...
- Server host-policy native validation includes `octaryn_server_host_policy_probe`, a C++ probe that validates server/client module-disable environment policy, live process-stream mode detection, native startup host-frame construction, and native live process-stream callback loop stop behavior. Managed `Host` is orchestration glue over the loadable `octaryn_server_host` owner library, which is copied into the server bundle and resolved with `OCTARYN_SERVER_HOST_LIBRARY` when injected by validation.
- Server world-time native validation includes `octaryn_server_world_time_probe`, a C++ probe that validates the server-owned native clock/default snapshot/date carry/frame tick advancement/calendar/blob semantics mapped from old `core/world_time`.
- Server authority-tick native validation includes `octaryn_server_authority_tick_probe`, a C++ probe that validates the server-owned native schedule plan for client command drain before player simulation before world-time advancement, worker dependency waves, and callback failure short-circuiting. Managed `AuthorityTickRunner` is callback/interop glue over the loadable `octaryn_server_authority_tick` owner library, which is copied into the server bundle and injected into managed validation probes with `OCTARYN_SERVER_AUTHORITY_TICK_LIBRARY`.
- Server block-store native validation includes `octaryn_server_block_store_probe`, a C++ probe that validates centered world bounds, floor chunk/local mapping, override edits, preserved air overrides, deterministic snapshots, chunk-column snapshots, load semantics, generated-override cleanup, native block-edit apply/support/cascade semantics, native chunk stream count/fill semantics, block-change queue drain semantics, client block-command queue semantics, client block-interaction hit/reach/adjacency semantics, and replication-change packing. The managed server block store/block edit service/change queue/client command queue/chunk stream provider are narrow native-handle glue over the loadable `octaryn_server_block_store` owner library, which is copied into the server bundle and injected into managed validation probes with `OCTARYN_SERVER_BLOCK_STORE_LIBRARY`.
- Server player-simulation native validation includes `octaryn_server_player_simulation_probe`, a C++ probe that validates native default spawn state, saved-state loading and save-change thresholds, spawn surface alignment, old-architecture walk/fly movement constants, jump/gravity behavior, AABB wall collision, and idle/no-input velocity clearing. The managed server player controller now uses this native owner library for default state, movement/collision, save-change decisions, and idle updates while keeping persistence and live-debug logging in server glue.
...
- Server world-persistence native validation includes `octaryn_server_world_persistence_probe`, a C++ probe that validates native edited-block chunk-column origin planning, per-column block ordering, chunk override directory scanning, stale sidecar pruning, sidecar directory writing, world-block aggregate/sidecar load-source and save coordination, save-import world-block writes, save-export chunk loading/planning, aggregate chunk-column counting, gzip round trips, player save JSON read/write, player directory scan/export ordering, player directory entry writes, world-time JSON read/write, and world metadata JSON read/write behavior. The same loadable `octaryn_server_world_persistence` owner library is copied into the server bundle and injected into managed validation probes with `OCTARYN_SERVER_WORLD_PERSISTENCE_LIBRARY`.
