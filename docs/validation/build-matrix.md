...
- Server authority-tick native validation includes `octaryn_server_authority_tick_probe`, a C++ probe that validates the server-owned native schedule plan for player simulation before world-time advancement, worker dependency waves, and callback failure short-circuiting. Managed `AuthorityTickRunner` is callback/interop glue over the loadable `octaryn_server_authority_tick` owner library, which is copied into the server bundle and injected into managed validation probes with `OCTARYN_SERVER_AUTHORITY_TICK_LIBRARY`.
...
- Server terrain-generation native validation includes `octaryn_server_terrain_generation_probe`, a C++ probe that validates native generated-block selection, floor chunk-local mapping, finite native noise sampling, and native empty-world sampling. The managed server terrain generator is narrow rules-callback glue over the loadable `octaryn_server_terrain_generation` owner library, which is copied into the server bundle and injected into managed validation probes with `OCTARYN_SERVER_TERRAIN_GENERATION_LIBRARY`.
- Server world-persistence native validation includes `octaryn_server_world_persistence_probe`, a C++ probe that validates native edited-block chunk-column origin planning, per-column block ordering, chunk override directory scanning, stale sidecar pruning, sidecar directory writing, world-block aggregate/sidecar load-source and save coordination, save-import world-block writes, aggregate chunk-column counting, gzip round trips, player save JSON read/write, player directory scan/export ordering, player directory entry writes, world-time JSON read/write, and world metadata JSON read/write behavior. The same loadable `octaryn_server_world_persistence` owner library is copied into the server bundle and injected into managed validation probes with `OCTARYN_SERVER_WORLD_PERSISTENCE_LIBRARY`.
