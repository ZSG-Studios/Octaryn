...
tools/build/cmake_build.sh debug-linux --target octaryn_validate_client_voxel_indirect_probe
...
- The graphical client launcher is currently a client-owned Slang RHI-backed bootstrap copied into `build/<preset>/client/bundle/` as `Octaryn.Client`. `octaryn_validate_client_app_launch_probe` proves the old SDL GPU / GLSL renderer is absent, the client reports the Slang RHI cutover stage honestly, creates a Slang device, validates the offscreen frame lifecycle, emits retained live-sidecar voxel frame-loop evidence, and records swapchain status. Local X11 swapchain presentation is a capability/status line only; radius-32 renderer proof remains the focused offscreen world-frame-loop probe until app launch/runtime coverage can use a radius-32 sidecar without unbounded server readiness work.
...
- Runtime/profiling acceptance for renderer work is defined by root `plan.md`. Historical SDL GPU mesh-upload evidence no longer counts for the active renderer; future proof must exercise the Slang RHI GPU voxel path, including bounded streaming/staging, retained GPU resources, indirect draw counts, VRAM/staging memory, pass timings, and no server JSON churn.
...
- Client voxel invariant validation includes `octaryn_client_voxel_invariants_probe`, a C++ probe that validates the replacement GPU-driven voxel renderer guardrails: 32³ chunks, 32-chunk columns, 1024-block world height, render-distance settings from 4 through 128 with no hidden outer buffer, Chebyshev column-ring priority, compact chunk-palette fast paths, column streaming budgets, and `PackedVoxelQuad16` encode/decode. Native host-executed probes run on Linux/x64 and are skipped in cross-target validate graphs.
- Client voxel mesh validation includes `octaryn_client_voxel_mesh_probe`, a C++ probe that validates empty chunks emit zero faces, solid chunks expose only the exterior shell, checkerboard chunks expose every solid face, and solid shell output packs to six `PackedVoxelQuad16` records.
- Server host-policy native validation includes `octaryn_server_host_policy_probe`, a C++ probe that validates server/client module-disable environment policy, live process-stream mode detection, metadata-only process-stream flag parsing, live stream path lookup, live stream request gating, native startup host-frame construction, and native live process-stream callback loop stop behavior. Managed `Host` and process-stream code are orchestration/log glue over the loadable `octaryn_server_host` owner library, which is copied into the server bundle and resolved with `OCTARYN_SERVER_HOST_LIBRARY` when injected by validation.
...
- Server terrain-generation native validation includes `octaryn_server_terrain_generation_probe`, a C++ probe that validates native terrain column planning, generated-block selection, floor chunk-local mapping, material block rules, water fill, and native empty-world sampling. Managed activation and validation probes call the loadable `octaryn_server_terrain_generation` owner library through `NativeTerrainGenerationLibrary` for material-rule interop, generated-block sampling, and generated-override cleanup; the library is copied into the server bundle and injected into managed validation probes with `OCTARYN_SERVER_TERRAIN_GENERATION_LIBRARY`.
