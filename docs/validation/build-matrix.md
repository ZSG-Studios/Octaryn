...
- Native jobs validation includes `octaryn_native_jobs_probe`, a C++ probe that validates worker-policy limits, scheduler resource conflicts, main-thread blocking policy, and Taskflow dependency/barrier execution through the native jobs target. Native host-executed probes run on Linux/x64 and are skipped in cross-target validate graphs.
