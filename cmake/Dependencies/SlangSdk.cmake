...
foreach(required_file IN ITEMS
    ...
    if(NOT EXISTS "${required_file}")
        message(FATAL_ERROR "Standalone slang-rhi prerequisite missing: ${required_file}. Run python tools/build/windows.py --action rhi --preset release-windows, then configure again.")
    endif()
endforeach()
...
if(NOT OCTARYN_SLANG_RHI_BUILT_COMMIT STREQUAL OCTARYN_SLANG_RHI_COMMIT OR NOT sdk_root STREQUAL built_sdk_root)
    message(FATAL_ERROR "Standalone slang-rhi build uses a different source pin or Slang SDK. Re-run python tools/build/windows.py --action rhi --preset release-windows with the selected SDK.")
endif()
...
if((CMAKE_BUILD_TYPE STREQUAL "Debug" AND NOT OCTARYN_SLANG_RHI_BUILT_CONFIG STREQUAL "Debug") OR
   (NOT CMAKE_BUILD_TYPE STREQUAL "Debug" AND OCTARYN_SLANG_RHI_BUILT_CONFIG STREQUAL "Debug"))
    message(FATAL_ERROR "Client and standalone slang-rhi CRT configurations differ. Run python tools/build/windows.py --action rhi --preset ${OCTARYN_BUILD_PRESET_NAME} and select its build directory.")
endif()
...
foreach(runtime_name IN ITEMS dxcompiler dxil)
    ...
    if(NOT EXISTS "${runtime_file}")
        message(FATAL_ERROR "DX12 compiler runtime missing: ${runtime_file}. Run python tools/build/windows.py --action rhi --preset release-windows.")
    endif()
    list(APPEND OCTARYN_CLIENT_SLANG_RUNTIME_FILES "${runtime_file}")
endforeach()
